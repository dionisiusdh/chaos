...
void isFolder(char *directoryTable, char *path, int *returncode, char current_dir)
{
    int targetEntry;
    int targetRes = 0;
    char targetIndex = directoryEvaluator(directoryTable, path, &targetRes, current_dir);

    if (targetRes == 0) {
        targetEntry = directoryTable[targetIndex * FILES_ENTRY_SIZE + ENTRY_BYTE_OFFSET];
        if (targetEntry == 0xFF) {
            *returncode = 1;
        } else {
            *returncode = 0;
...
void rm(char *directory_table, char *foldername, char current_dir_index) {
    int returncode;
    char files[1024];
    int idxRow;
    int idx;
    ...
    idxRow = directoryEvaluator(directory_table, foldername, returncode, current_dir_index);
    ...
    {
        if (i == 0)
            files[(idxRow<<4)+i] = 0xFF;
        else if (i == 1)
            files[(idxRow<<4)+i] = 0xFE;
        else
...
void cp(char *src, char *dest, int argc, char *directoryTable, char current_dir_index) {
    char buffer_content[FILE_SIZE_MAXIMUM];
    char filename1[14];
    int returncode = 0;
    int returncode1, returncode2;
    int dirsrc, dirdest;
    int parentsrc, parentdest;
    int isSrcFolder, isDestFolder;

    isFolder(directoryTable, src, isSrcFolder, current_dir_index);
    isFolder(directoryTable, dest, isDestFolder, current_dir_index);

    if (isSrcFolder && !isDestFolder) {
        print("Cannot copy folder into a file!\n", BIOS_GRAY);
    } else { 
        if (argc == 3) {
            dirsrc = directoryEvaluator(directoryTable, src, &returncode1, current_dir_index);
            dirdest = directoryEvaluator(directoryTable, dest, &returncode2, current_dir_index);

            if (returncode1 == 0 && returncode2 == 0) {
                parentsrc = directoryTable[dirsrc * FILES_ENTRY_SIZE + PARENT_BYTE_OFFSET];
                parentdest = directoryTable[dirdest * FILES_ENTRY_SIZE + PARENT_BYTE_OFFSET];

                clear(filename1, 14);
                strcpybounded(filename1, src, 14);

                clear(buffer_content, FILE_SIZE_MAXIMUM);
                readFile(buffer_content, filename1, &returncode, parentsrc);

                if (returncode == -1) {
                    print("cp: ", BIOS_GRAY);
                    print(filename1, BIOS_GRAY);
                    print(": not found\n", BIOS_GRAY);
                } else {
                    rm(directoryTable, dest, parentdest);
                    write(buffer_content, dest, &returncode, parentdest);
                }
            } else {
                if (returncode1 != 0) {
                    print("cp: ", BIOS_GRAY);
...
void mv (char *src, char *dest, int argc, char *directoryTable, char current_dir_index) {
    int i;
    int returncode1, returncode2;
    int isSrcFolder, isDestFolder;
    char buffer[512 * 16];
    ...
    if (isSrcFolder && !isDestFolder) {
        print("Cannot move folder into a file!\n", BIOS_GRAY);
    } else {
        if (argc == 3) {
            cp(src, dest, 3, directoryTable, current_dir_index);
            rm(directoryTable, src, current_dir_index);
        } else {
...
void shell() {
    ...
    while (true) {
        ...
        if (!strcmp("ls", arg_vector[0]))  {
        ...
        else if (!strcmp("mv", arg_vector[0])) {
            if (argc == 3) {
                mv(arg_vector[1], arg_vector[2], argc, directory_table, current_dir_index);
                getDirectoryTable(directory_table); // Reload
            }
            else
                print("Usage : mv <filename> <dirname>\n", BIOS_WHITE);
        }
        else if (!strcmp("cp", arg_vector[0])) {
            if (argc == 3) {
                cp(arg_vector[1], arg_vector[2], argc, directory_table, current_dir_index);
                getDirectoryTable(directory_table); // Reload
            }
            else
                print("Usage : cp <src> <dest>\n", BIOS_WHITE);
        }
        else if (!strcmp("rm", arg_vector[0])) {
            if (argc == 2) {
                rm(directory_table, arg_vector[1], current_dir_index);
                getDirectoryTable(directory_table); // Reload
            }
            else
                print("Usage : rm <name>\n", BIOS_WHITE);
            ...
            if (argc == 2) {
                executeProgram(arg_vector[1], 0x5000, &success, 0xFF);
            } else {
